		...
		if err != nil {
			...
			if dimErr != nil {
				...
				if ffmpegErr != nil {
					...
					cmd = exec.Command("ffmpeg", "-i", filePath, "-vcodec", "png", "-frames:v", "1", tempPngPath)
					ffmpegOutput, ffmpegErr = cmd.CombinedOutput()
					if ffmpegErr != nil {
						// If that also fails, try with -pix_fmt and avoiding hardware acceleration
						logger.Printf("WARN: Second ffmpeg attempt failed for %s: %v. Output: %s. Trying third approach.", filepath.Base(filePath), ffmpegErr, string(ffmpegOutput))
				...
				if width > 0 && height > 0 {
					...
					if ffmpegErr != nil {
						...
						ffmpegOutput, ffmpegErr = cmd.CombinedOutput()
						if ffmpegErr != nil {
							logger.Printf("WARN: Unscaled ffmpeg method also failed for %s: %v. Output: %s. Trying with different pixel format.", filepath.Base(filePath), ffmpegErr, string(ffmpegOutput))
							// Try with different pixel format and no hardware acceleration
							cmd = exec.Command("ffmpeg", "-hwaccel", "none", "-i", filePath, "-pix_fmt", "yuv420p", "-frames:v", "1", "-c:v", "png", tempPngPath)
							ffmpegOutput, ffmpegErr = cmd.CombinedOutput()
						}
					}
				} else {
			...
			if ffmpegErr != nil {
				...
				tempRelaxedPngPath := tempJxlPath + ".relaxed.png"
				...
				cmd = exec.Command("cjxl", tempRelaxedPngPath, tempJxlPath, "-d", "0", "-e", "9", "--num_threads", strconv.Itoa(opts.CJXLThreads))
			} else {
