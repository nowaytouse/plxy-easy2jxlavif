...
func isSupportedFile(ext string) bool {
	// video2mov只处理视频文件，排除所有图片

	// 仅支持视频格式
	supportedExts := map[string]bool{
		".mov": true, ".mp4": true, ".avi": true, ".mkv": true,
		".webm": true, ".m4v": true, ".mpg": true, ".mpeg": true,
		".wmv": true, ".flv": true, ".mp2": true, ".mpe": true,
		".mpv": true, ".m2v": true, ".3gp": true, ".3g2": true,
	}
...
func processFileByType(filePath string, opts Options) (string, string, string, error) {
	...
	if output, err := cmd.CombinedOutput(); err != nil {
		return conversionMode, "", string(output), fmt.Errorf("ffmpeg重封装失败: %v", err)
	}

	logger.Printf("✅ 视频重封装成功（内部元数据已保留）: %s", filepath.Base(outputPath))
