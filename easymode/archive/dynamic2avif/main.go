...
func processDirectory(ctx context.Context, opts *Options, stats *Stats) ([]string, error) {
	...
	err := godirwalk.Walk(opts.InputDir, &godirwalk.Options{
		Callback: func(osPathname string, de *godirwalk.Dirent) error {
			...
			// 检查文件类型
			kind, _ := filetype.MatchFile(osPathname)
			ext := strings.ToLower(filepath.Ext(osPathname))
			if isSupportedDynamicType(kind) || ext == ".heic" || ext == ".heif" {
	...
	sort.SliceStable(files, func(i, j int) bool {
		return utils.GetFileProcessingPriority(files[i]) > utils.GetFileProcessingPriority(files[j])
	})
	...
	// 统一的内存感知线程调整
	workers = utils.SmartThreadAdjustment(workers)
...
func convertToAvifWithOpts(filePath, outputPath string, opts *Options) error {
	...
	validator := utils.NewEightLayerValidator(utils.ValidationOptions{StrictMode: true, AllowTolerance: 0.01, TimeoutSeconds: opts.TimeoutSeconds, CJXLThreads: runtime.NumCPU()})
	if vr, vErr := validator.ValidateConversion(filePath, outputPath, enhancedType); vErr != nil {
...
func isSupportedDynamicType(kind types.Type) bool {
	return supportedDynamicExtensions[kind.Extension]
}
...
func isAnimatedImage(filePath string) (bool, error) {
	...
	case "heic", "heif":
		// For HEIC/HEIF, use extension-based detection since filetype library might not detect them properly
		return isAnimatedHEIF(filePath)
	default:
		// Also try extension-based detection for HEIC/HEIF files
		if ext == ".heic" || ext == ".heif" {
			return isAnimatedHEIF(filePath)
		}
		return false, nil
...
func validateFileCount(workDir string, originalMediaCount int, stats *Stats) {
	...
	if currentAvifCount == expectedAvifCount && currentMediaCount == expectedMediaCount {
		...
		logger.Printf("⚠️ 文件数量验证存在差异 (实际AVIF: %d, 期望AVIF: %d; 实际剩余: %d, 期望剩余: %d) —— 仅记录，不判失败。", currentAvifCount, expectedAvifCount, currentMediaCount, expectedMediaCount)
	}
