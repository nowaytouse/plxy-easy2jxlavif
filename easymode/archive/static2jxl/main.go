...
func parseFlags() *Options {
	...
	if opts.CJXLThreads < 1 || opts.CJXLThreads > 32 {
		logger.Fatal("❌ 错误: CJXL线程数必须在1-32之间")
	}
...
func processFile(ctx context.Context, filePath string, opts *Options) FileProcessInfo {
	...
	if utils.IsLivePhoto(filePath) {
		logger.Printf("🏞️  检测到苹果Live Photo，跳过: %s", fileName)
...
// smartThreadAdjustment dynamically adjusts workers based on memory usage
...
// getFileProcessingPriority returns priority based on file type (higher = process first)
