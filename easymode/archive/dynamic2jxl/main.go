...
func processFileByType(filePath string, opts Options) (string, string, string, error) {
	...
	// 检测是否为动图（简化版）
	isAnimated := strings.ToLower(filepath.Ext(filePath)) == ".gif"
...
func runInteractiveMode() {
	...
	fmt.Println("╔═══════════════════════════════════════════════════════════════╗")
	fmt.Println("║                                                               ║")
	fmt.Println("║   🎨 dynamic2jxl v2.3.0 - 动图转JXL工具                    ║")
	fmt.Println("║                                                               ║")
	fmt.Println("║   功能: 动态图片转换为JXL格式（无损动图压缩）              ║")
...
func runNonInteractiveMode_WithOpts(opts Options) {
	logger.Printf("🎨 dynamic2jxl v%s", version)
	logger.Println("🔍 检查系统依赖...")
	if err := checkDependencies(); err != nil {
		logger.Fatalf("❌ 系统依赖检查失败: %v", err)
	}
