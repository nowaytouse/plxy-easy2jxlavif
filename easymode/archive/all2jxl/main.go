...
func parseFlags() Options {
	...
	flag.StringVar(&outputDir, "output", "", "📁 输出目录（默认为输入目录）")
	...
	if workers < 0 || workers > 100 {
		logger.Fatalf("❌ 工作线程数必须在0-100之间")
	}
	if cjxlThreads < 1 || cjxlThreads > 32 {
		logger.Fatalf("❌ CJXL线程数必须在1-32之间")
	}
	if timeoutSec < 0 || timeoutSec > 3600 {
		logger.Fatalf("❌ 超时时间必须在0-3600秒之间")
	}
	if retries < 0 || retries > 10 {
		logger.Fatalf("❌ 重试次数必须在0-10之间")
	}
	if sample < 0 || sample > 10000 {
		logger.Fatalf("❌ 采样数量必须在0-10000之间")
	}
	...
	return Options{Workers: workers, Verify: vm, DoCopy: doCopy, Sample: sample, SkipExist: skipExist, DryRun: dryRun, CJXLThreads: cjxlThreads, TimeoutSeconds: timeoutSec, Retries: retries, InputDir: dir, OutputDir: outputDir}
}
...
func validateFileType(filePath string) error {
	...
	// 特殊处理：某些格式可能无法被filetype库正确识别
	if kind == types.Unknown {
		// 允许某些特殊格式通过扩展名判断
		specialFormats := map[string]bool{".ico": true, ".cur": true, ".jfif": true, ".jpe": true, ".avif": true, ".heic": true, ".heif": true}
		...
		if ext != expectedExt && !isCompatibleExtension(ext, expectedExt) {
			// 特殊处理AVIF文件，即使filetype无法识别也允许通过
			if ext == ".avif" || ext == ".heic" || ext == ".heif" {
				// 这些格式可能无法被filetype正确识别，但扩展名正确就允许通过
			} else {
...
func processFileWithOpts(filePath string, fileInfo os.FileInfo, stats *Stats, opts Options) {
	...
	if err != nil {
		logger.Printf("⏭️  文件类型检测失败: %s - %v", fileName, err)
		processInfo.ErrorMsg = "文件类型检测失败"
		processInfo.ProcessingTime = time.Since(startTime)
		stats.addDetailedLog(processInfo)
		stats.addOtherSkipped()
		return
	...
	logger.Printf("🔍 文件类型信息: %s - %s", fileName, utils.GetFileTypeInfo(filePath))

	// 检查是否为支持的图像格式
	if !enhancedType.IsImage || !enhancedType.IsValid {
		logger.Printf("⏭️  不支持的文件类型: %s (类型: %s)", fileName, enhancedType.Extension)
		processInfo.ErrorMsg = "不支持的文件类型"
		processInfo.ProcessingTime = time.Since(startTime)
		stats.addDetailedLog(processInfo)
		stats.addOtherSkipped()
		return
	...
	if enhancedType.IsAnimated {
		...
		processInfo.IsAnimated = true
	}

	logger.Printf("✅ 识别为图像格式: %s (%s)", fileName, enhancedType.Extension)
	...
	// 苹果Live Photo检测
	if enhancedType.Extension == "heic" || enhancedType.Extension == "heif" {
	...
	for attempt := 0; attempt <= opts.Retries; attempt++ {
		...
		conversionMode, jxlPath, tempJxlPath, err = convertToJxlWithOpts(filePath, enhancedType, opts)
		if err != nil {
	...
	verified, err := verifyConversionWithMode(filePath, tempJxlPath, enhancedType, opts)
	if err != nil {
	...
	// 统一8层验证补充（与内部验证口径一致，增强反作弊）
	validator := utils.NewEightLayerValidator(utils.ValidationOptions{StrictMode: true, AllowTolerance: 0.01, TimeoutSeconds: opts.TimeoutSeconds, CJXLThreads: opts.CJXLThreads})
	vr, vErr := validator.ValidateConversion(filePath, tempJxlPath, enhancedType)
	if vErr != nil {
		logger.Printf("❌ 验证失败 %s: %v", fileName, vErr)
		os.Remove(tempJxlPath)
		processInfo.ErrorMsg = fmt.Sprintf("验证失败: %v", vErr)
		processInfo.ProcessingTime = time.Since(startTime)
		stats.addDetailedLog(processInfo)
		stats.addImageFailed()
		return
	} else if !vr.Success {
		logger.Printf("❌ 验证失败 %s: %s (第%d层: %s)", fileName, vr.Message, vr.Layer, vr.LayerName)
		os.Remove(tempJxlPath)
		processInfo.ErrorMsg = fmt.Sprintf("验证失败: %s", vr.Message)
		processInfo.ProcessingTime = time.Since(startTime)
		stats.addDetailedLog(processInfo)
		stats.addImageFailed()
		return
	}
	logger.Printf("✅ 验证通过: %s (%s)", fileName, vr.Message)
...
func isVideoType(enhancedType utils.EnhancedFileType) bool {
	return strings.HasPrefix(enhancedType.MIME, "video")
}

// isAnimatedImage 检测是否为真实的动画图像
func isAnimatedImage(filePath string, enhancedType utils.EnhancedFileType) (bool, error) {
	switch enhancedType.Extension {
	case "gif":
...
func convertToJxlWithOpts(filePath string, enhancedType utils.EnhancedFileType, opts Options) (string, string, string, error) {
	...
	// 使用增强的文件类型信息
	actualExt := enhancedType.Extension
	...
	case "avif":
		// AVIF需要预处理，cjxl无法直接处理
		return convertAVIFWithPreprocessing(filePath, tempJxlPath, opts)
	case "bmp":
	...
	case "tiff", "tif":
		...
		cmd = exec.Command("cjxl", filePath, tempJxlPath, "-d", "0", "-e", "9", "--num_threads", strconv.Itoa(opts.CJXLThreads))
	default:
		return "", "", "", fmt.Errorf("unhandled supported type: %s", enhancedType.Extension)
	}
...
// convertAVIFWithPreprocessing 使用ImageMagick预处理AVIF文件后转换为JXL
func convertAVIFWithPreprocessing(filePath, tempJxlPath string, opts Options) (string, string, string, error) {
	// 生成正确的JXL路径
	jxlPath := strings.TrimSuffix(filePath, filepath.Ext(filePath)) + ".jxl"
	tempPngPath := tempJxlPath + ".png"
	defer os.Remove(tempPngPath) // 清理临时PNG文件

	// 使用ImageMagick将AVIF转换为PNG
	cmd := exec.Command("magick", filePath, tempPngPath)
	output, err := cmd.CombinedOutput()
	if err != nil {
		// 如果ImageMagick失败，尝试使用ffmpeg
		logger.Printf("WARN: ImageMagick failed for %s: %v. Trying ffmpeg.", filepath.Base(filePath), err)
		cmd = exec.Command("ffmpeg", "-i", filePath, "-y", tempPngPath)
		output, err = cmd.CombinedOutput()
		if err != nil {
			return "", "", "", fmt.Errorf("AVIF预处理失败: %v, 输出: %s", err, string(output))
		}
	}
	...
	output, err = cmd.CombinedOutput()
	if err != nil {
		return "", "", "", fmt.Errorf("cjxl转换失败: %v, 输出: %s", err, string(output))
	}

	return "AVIF Lossless Conversion (via PNG)", jxlPath, tempJxlPath, nil
}
...
func verifyConversionWithMode(originalPath, tempJxlPath string, enhancedType utils.EnhancedFileType, opts Options) (bool, error) {
	...
	// 对于所有格式都进行严格验证，包括AVIF/HEIC/HEIF
	if ext == ".heic" || ext == ".heif" || ext == ".avif" {
		logger.Printf("INFO: [HEIC/AVIF] Performing strict verification for %s.", originalPath)

		// 第一步：验证JXL文件可以正确解码
		// 确保使用绝对路径
		absTempJxlPath, err := filepath.Abs(tempJxlPath)
		if err != nil {
			return false, fmt.Errorf("could not get absolute path for %s: %w", tempJxlPath, err)
		}

		// 检查文件是否存在
		if _, err := os.Stat(absTempJxlPath); err != nil {
			return false, fmt.Errorf("JXL file does not exist: %s", absTempJxlPath)
		}

		// 调试信息：打印实际路径
		logger.Printf("DEBUG: Verifying JXL file: %s", absTempJxlPath)

		decodedPNGPath := filepath.Join(tempDir, "decoded.png")
		ctx, cancel := withTimeout(context.Background(), opts)
		defer cancel()
		procSem <- struct{}{}
		decodeCmd := exec.CommandContext(ctx, "djxl", absTempJxlPath, decodedPNGPath, "--num_threads", strconv.Itoa(opts.CJXLThreads))
		decodeOutput, err := decodeCmd.CombinedOutput()
		<-procSem
		if err != nil {
			return false, fmt.Errorf("djxl execution failed for %s: %w\nOutput: %s", absTempJxlPath, err, string(decodeOutput))
		}
		...
		// 第二步：质量优先的验证 - 平衡严格性和实用性
		// 对于AVIF/HEIC/HEIF，我们进行多层次的验证

		// 2.1 基础验证：确保JXL文件可解码且非空
		if fi, statErr := os.Stat(decodedPNGPath); statErr != nil || fi.Size() == 0 {
			return false, fmt.Errorf("djxl produced an empty or missing file for %s", absTempJxlPath)
		}

		// 2.2 尺寸验证：检查原始文件和JXL解码后的尺寸
		decodedImg, _, err := readImage(decodedPNGPath)
		if err != nil {
			return false, fmt.Errorf("could not decode JXL output %s: %w", decodedPNGPath, err)
		}

		// 2.3 尝试获取原始文件尺寸进行对比
		originalDims, err := getImageDimensions(originalPath)
		if err != nil {
			logger.Printf("WARN: Could not get original dimensions for %s: %v", originalPath, err)
			// 如果无法获取原始尺寸，只验证JXL文件本身的质量
			logger.Printf("INFO: [HEIC/AVIF] Basic verification successful for %s (JXL decode ok, size: %dx%d)",
				originalPath, decodedImg.Bounds().Dx(), decodedImg.Bounds().Dy())
			return true, nil
		}

		// 2.4 尺寸对比验证
		if originalDims.Width != decodedImg.Bounds().Dx() || originalDims.Height != decodedImg.Bounds().Dy() {
			logger.Printf("FAIL: Dimension mismatch for %s: original=%dx%d, decoded=%dx%d",
				filepath.Base(originalPath), originalDims.Width, originalDims.Height,
		...
		if err == nil {
			jxlSize, err := getFileSize(absTempJxlPath)
			if err == nil {
				// 对于AVIF/HEIC，JXL文件大小应该在合理范围内
				sizeRatio := float64(jxlSize) / float64(originalSize)
				if sizeRatio > 10.0 { // JXL文件不应该比原文件大10倍以上
					logger.Printf("WARN: JXL file unusually large for %s: ratio=%.2f", originalPath, sizeRatio)
				}
				logger.Printf("INFO: File size ratio for %s: %.2f (original: %d bytes, JXL: %d bytes)",
					filepath.Base(originalPath), sizeRatio, originalSize, jxlSize)
			}
		}

		logger.Printf("INFO: [HEIC/AVIF] Strict verification successful for %s.", originalPath)
		return true, nil
	}

	// 使用临时目录承载解码输出
	// tempDir已经在上面声明了，这里不需要重新声明
	defer os.RemoveAll(tempDir)
	...
	if isAnimated {
		...
		logger.Printf("INFO: %s verified on first frame; frame count=%d; timing/disposal not verified due to decoder limits.", enhancedType.Extension, jxlFrames)
		return true, nil
	}
...
// calculateImageDifference 计算两个图像之间的差异百分比
func calculateImageDifference(img1, img2 image.Image) float64 {
	bounds1 := img1.Bounds()
	bounds2 := img2.Bounds()

	if bounds1 != bounds2 {
		return 100.0 // 尺寸不同，差异100%
	}
	...
	for y := bounds1.Min.Y; y < bounds1.Max.Y; y++ {
		for x := bounds1.Min.X; x < bounds1.Max.X; x++ {
			...
			// 计算颜色差异（允许1的误差）
			if abs(int(r1)-int(r2)) > 1 || abs(int(g1)-int(g2)) > 1 ||
				abs(int(b1)-int(b2)) > 1 || abs(int(a1)-int(a2)) > 1 {
				differentPixels++
			}
		}
	}

	return float64(differentPixels) / float64(totalPixels) * 100.0
}
...
// ImageDimensions 存储图像尺寸信息
type ImageDimensions struct {
	Width  int
	Height int
}

// getImageDimensions 获取图像文件的尺寸
func getImageDimensions(filePath string) (ImageDimensions, error) {
	// 使用exiftool获取图像尺寸
	cmd := exec.Command("exiftool", "-q", "-q", "-ImageWidth", "-ImageHeight", filePath)
...
func validateFileCount(workDir string, originalMediaCount int, stats *Stats) {
	...
	if currentJxlCount == expectedJxlCount && currentMediaCount == expectedMediaCount {
		...
		logger.Printf("⚠️ 文件数量验证存在差异 (实际JXL: %d, 期望JXL: %d; 实际剩余: %d, 期望剩余: %d) —— 仅记录，不判失败。", currentJxlCount, expectedJxlCount, currentMediaCount, expectedMediaCount)
	}
	...
	if currentMediaCount != expectedMediaCount {
		logger.Printf("ℹ️  目录包含未处理原文件，忽略为提示。")
	}
...
// In main, after files = scanCandidateFiles, sort
sort.SliceStable(files, func(i, j int) bool {
	return getFileProcessingPriority(files[i]) > getFileProcessingPriority(files[j])
})

// After workers calculation
