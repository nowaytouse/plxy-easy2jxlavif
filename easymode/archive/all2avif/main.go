...
func main() {
	...
	if err != nil {
		logger.Fatalf("❌ 无效的输入目录路径: %v", err)
	}
	opts.InputDir = absInputDir
...
func parseFlags() Options {
	...
	if *workers < 1 || *workers > 100 {
		logger.Fatalf("❌ 工作线程数必须在1-100之间")
	}
	if *quality < 1 || *quality > 100 {
		logger.Fatalf("❌ 图像质量必须在1-100之间")
	}
	if *speed < 0 || *speed > 6 {
		logger.Fatalf("❌ 编码速度必须在0-6之间")
	}
	if *timeoutSec < 1 || *timeoutSec > 3600 {
		logger.Fatalf("❌ 超时时间必须在1-3600秒之间")
	}
...
func validateFileType(filePath string) error {
	...
	// 检查文件大小是否合理（1字节到1GB）
	if fileInfo.Size() < 1 || fileInfo.Size() > 1024*1024*1024 {
	...
	if ext == ".heic" || ext == ".heif" {
		return validateHEICFile(filePath, head[:n])
	}
...
func validateHEICFile(filePath string, header []byte) error {
	...
	if err != nil {
		return fmt.Errorf("无法获取HEIC文件信息: %v", err)
	}

	// HEIC文件最小应该大于1KB
	if fileInfo.Size() < 1024 {
		return fmt.Errorf("HEIC文件过小，可能损坏")
	}

	// 使用exiftool验证HEIC文件
	cmd := exec.Command("exiftool", "-q", "-q", filePath)
	if err := cmd.Run(); err != nil {
		return fmt.Errorf("HEIC文件验证失败: %v", err)
	}
...
// isLivePhoto 检查是否为Live Photo文件
// Live Photo通常包含特定的元数据标识
func isLivePhoto(filePath string) bool {
	// 使用exiftool检查是否为Live Photo
	cmd := exec.Command("exiftool", "-q", "-q", "-LivePhotoVideo", filePath)
	output, err := cmd.Output()
	if err != nil {
		return false
	}

	// 如果输出包含LivePhotoVideo标签，则为Live Photo
	return strings.Contains(string(output), "LivePhotoVideo")
}

// getFileProcessingPriority 获取文件处理优先级
// 优先级：JPEG > PNG > HEIC/HEIF > 其他格式
...
func smartThreadAdjustment(baseWorkers int, fileType string) int {
	...
	if memUsagePercent > 80 {
		return max(1, baseWorkers/2) // 内存使用过高，减少线程
	} else if memUsagePercent > 60 {
		return max(1, int(float64(baseWorkers)*0.7)) // 内存使用较高，适度减少
	}

	// 根据文件类型调整
	switch strings.ToLower(fileType) {
	case ".heic", ".heif":
		return min(baseWorkers, cpuCount) // HEIC处理需要更多资源
	case ".jpg", ".jpeg":
		return min(baseWorkers*2, cpuCount*2) // JPEG处理较快，可以增加线程
	default:
		return min(baseWorkers, cpuCount)
	}
}
...
func processFileWithOpts(filePath string, opts Options, stats *Stats) {
	...
	if err := validateFileType(filePath); err != nil {
		logger.Printf("⚠️  文件验证失败 %s: %v", filepath.Base(filePath), err)
		stats.addOtherSkipped()
	...
	result, vErr := validator.ValidateConversion(filePath, avifPath, enhancedType)
	if vErr != nil {
		logger.Printf("❌ 验证失败 %s: %v", filepath.Base(filePath), vErr)
		_ = os.Remove(avifPath)
		stats.addFailure()
		return
	}
	if !result.Success {
		logger.Printf("❌ 验证失败 %s: %s (第%d层: %s)", filepath.Base(filePath), result.Message, result.Layer, result.LayerName)
		_ = os.Remove(avifPath)
		stats.addFailure()
		return
	}
	logger.Printf("✅ 验证通过: %s (%s)", filepath.Base(filePath), result.Message)
...
func validateFileCount(workDir string, originalMediaCount int, stats *Stats) {
	...
	if currentAvifCount == expectedAvifCount {
		...
		logger.Printf("⚠️  目标格式数量不匹配 (实际: %d, 期望: %d) —— 仅提示，不判失败。", currentAvifCount, expectedAvifCount)
	}
