...
func isSupportedFile(ext string) bool {
	// dynamic2jxl只处理动图（GIF/WEBP/APNG），明确排除视频和静态图
...
func processFileByType(filePath string, opts Options) (string, string, string, error) {
	...
	if utils.NeedsConversion(filePath, "cjxl") {
		...
		if err != nil {
			return conversionMode, "", "", fmt.Errorf("格式转换失败: %v", err)
		}
		if wasConverted {
	...
	// 检测是否为动图（简化版）
	isAnimated := strings.ToLower(filepath.Ext(actualInputPath)) == ".gif"
	...
	if !isAnimated {
		...
		cmd := exec.CommandContext(ctx, "cjxl", args...)
		if output, err := cmd.CombinedOutput(); err != nil {
			return conversionMode, "", string(output), fmt.Errorf("cjxl动图转换失败: %v", err)
		}
	}
