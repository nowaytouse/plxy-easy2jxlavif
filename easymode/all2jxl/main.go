...
func convertStaticToJxl(inputPath, outputPath string, opts Options) (string, string, string, error) {
	...
	needsCleanup := false
	if utils.NeedsConversion(inputPath, "cjxl") {
		convertedPath, wasConverted, err := utils.ConvertIfNeeded(inputPath, "cjxl")
		if err != nil {
			return "静态转换", outputPath, fmt.Sprintf("格式转换失败: %v", err), err
		}
		if wasConverted {
...
func convertAnimatedToJxl(inputPath, outputPath string, opts Options) (string, string, string, error) {
	...
	if utils.NeedsConversion(inputPath, "cjxl") {
		convertedPath, wasConverted, err := utils.ConvertIfNeeded(inputPath, "cjxl")
		if err != nil {
			return "动画转换", outputPath, fmt.Sprintf("格式转换失败: %v", err), err
		}
		if wasConverted {
			actualInputPath = convertedPath
			needsCleanup = true
