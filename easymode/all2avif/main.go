...
func processFileWithOpts(filePath string, opts Options, stats *Stats) {
	...
	if isAnimated {
		...
		processInfo.ConversionMode = "Animated Conversion"
	} else {
		logger.Printf("🖼️  静态图像: %s", filepath.Base(filePath))
		processInfo.ConversionMode = "Static Conversion"
	}
...
func convertToAvif(filePath string, kind types.Type, isAnimated bool, opts Options) (int64, error) {
	...
	// HEIC/HEIF conversion using magick to a more stable PNG intermediate
	if kind.Extension == "heic" || kind.Extension == "heif" {
		...
		if err != nil {
			return 0, fmt.Errorf("magick execution for HEIC failed: %s\nOutput: %s", err, string(output))
		}
		defer os.Remove(tempPngPath)
		filePath = tempPngPath // Use the new png file as input for ffmpeg
	}
